...
/**
 * Column layout for `columnContainer` blocks. Each column (`entries[]` item) is one
 * grid cell and may include, in order (all optional):
 * 1. **Headline** — from `headline` block when set; else entry `title` (see mapper)
...
import {
  spacingTokenToUseSpacersSize,
  useSpacers,
} from '~/composables/useSpacers';
import {parseColumnSpanUnits} from '~/composables/useColumnspan';
import type {SectionContainerEntry} from '~~/types';
import type {CSSProperties} from 'vue';

const props = withDefaults(
  defineProps<{
    entries: SectionContainerEntry[];
    /** `columnSpan` plain text on the column container entry (`6-6`, `4-8`, …). */
...
const containerClass = computed(() => {
  const marginTopClass = getSpacerTop(
    spacingTokenToUseSpacersSize(props.marginTop),
  );
  const marginBottomClass = getSpacerBottom(
    spacingTokenToUseSpacersSize(props.marginBottom),
  );
  const paddingTopClass = getPaddingTop(
    spacingTokenToUseSpacersSize(props.paddingTop),
  );
  const paddingBottomClass = getPaddingBottom(
    spacingTokenToUseSpacersSize(props.paddingBottom),
  );
...
const innerClass = computed(() => {
  const top = getPaddingTop(spacingTokenToUseSpacersSize(props.paddingTop));
...
const columnSpanUnitsList = computed(() =>
  parseColumnSpanUnits(props.columnSpan ?? null, props.entries.length),
);
      ...
      >
        ...
        >
          ...
          >
            ...
            <div
              v-if="
                sectionContainerEntry.linkHref &&
                sectionContainerEntry.linkLabel
              "
              :id="`block-${sectionContainerEntry.id}-link`"
              class="section-element section-element--link"
            >
              <Button
                :type="sectionContainerEntry.buttonType ?? 'primary'"
                :size="sectionContainerEntry.buttonSize ?? 'medium'"
                :href="sectionContainerEntry.linkHref"
                :target="sectionContainerEntry.linkTarget ?? undefined"
                :disabled="sectionContainerEntry.buttonDisabled ?? false"
                :loading="sectionContainerEntry.buttonLoading ?? false"
              >
                {{ sectionContainerEntry.linkLabel }}
              </Button>
            </div>
